...
def val_in_context_learning_a_ok_vqa_with_mcan(llama_model, 
                                               ...
                                               train_images_dir,
                                               val_images_dir,
                                               decomposed_caption_path,
                                               decomposed_knowledge_path, 
                                               n_shots=10, 
                                               k_ensemble=5,
                                               MAX_CAPTION_LEN=30, 
  ...
  :param decomposed_caption_path: The path of the decomposed captions
  :param decomposed_knowledge_path: The path of the decomposed knowledge
  ...
  for i in tqdm(range(val_annotations_df.shape[0])):
    ...
    for k in range(k_ensemble): # we use k promts for each test sample
      ...
      for ni in range(n_shots_var):
        ...
        #sort the captions based on the cos sim for in-context samples
      ...
      #get knowledge of the test sample
      test_sample_knowledge = val_captions[val_captions.question_id==test_sample.question_id].iloc[0].captions
      raw_test_image = Image.open(val_images_dir+test_sample.image_path)
      knowledge = open(decomposed_knowledge_path, 'r')
      knowledge = json.load(knowledge)
      know = knowledge[test_sample.question_id]
      for value in know.values():
        test_sample_knowledge.append(value)
 
      
      #sort the knowledge based on the cos sim 
      test_sample_knowledge, cos_scores = sort_captions_based_on_similarity(test_sample_knowledge,raw_image=raw_test_image,model=blip_model,processor=blip_processor,device = device, ascending = False)
      
      
      for j,caption in enumerate(test_sample_knowledge[:NO_OF_CAPTIONS_AS_CONTEXT]):
        caption = " ".join(caption.split()[:MAX_CAPTION_LEN]) #truncatte
        if j <= NO_OF_CAPTIONS_AS_CONTEXT-1:
          prompt += '%s,\n'%caption

      q = open(decomposed_caption_path, 'r')
      c = json.load(q)
      caption = c[test_sample.question_id]
      prompt += '%s\n'%caption
      ...
      outputs = llama_model.generate(input_ids, max_length=prompt_tokens + 5, num_beams=2, return_dict_in_generate=True, output_scores=True, num_return_sequences=1, do_sample=False, temperature = 1.0, top_p = 1.0)
...
def test_in_context_learning_a_ok_vqa_with_mcan(llama_model,
  ...
  :param blip_processor: The blip huggingface processor
  :param train_annotations_df: Dataframe containing the ok_vqa train annotations
  :param test_annotations_df: Dataframe containing the ok_vqa test annotations
  :param context_examples_df: Dataframe containing the mcan examples
  :param train_captions: Dataframe containing the train question-informative captions
  :param test_captions: Dataframe containing the test question-informative captions
  :param train_images_dir: The path of the folder containing the training images
  :param test_images_dir: The path of the folder containing the test images
  :param decomposed_caption_path: The path of the decomposed captions
  :param decomposed_knowledge_path: The path of the decomposed knowledge
  :param n_shots: The number of shots for the in-context few-shot learning
  :param k_ensemble: The number of ensembles
  ...
  for i in tqdm(range(test_annotations_df.shape[0])):
    ...
    for k in range(k_ensemble): # we use k promts for each test sample
      ...
      #get knowledge of the test sample
      test_sample_knowledge = test_captions[test_captions.question_id==test_sample.question_id].iloc[0].captions
      ...
      #get decomposed caption of the test sample
      q = open(decomposed_caption_path, 'r')
      c = json.load(q)
      caption = c[test_sample.question_id]
      prompt += '%s\n'%caption
