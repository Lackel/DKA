...
def val_in_context_learning_ok_vqa_with_mcan(llama_model, 
  ...
  :decomposed_caption_path: The path of the decomposed captions
  :decomposed_knowledge_path: The path of the decomposed knowledge
  ...
  for i in tqdm(range(val_annotations_df.shape[0])):
    ...
    for k in range(k_ensemble): # we use k promts for each test sample
      ...
      #get knowledge of the test sample (shot)
      ...
      know = knowledge[str(test_sample.question_id)]
      ...
      q = open(decomposed_knowledge_path, 'r')
      c = json.load(q)
      caption = c[str(test_sample.question_id)]
      prompt += '%s\n'%caption
